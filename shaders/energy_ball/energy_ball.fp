...
uniform energy_ball_fp {
    vec4 resolution;  // xy: resolution, z: texture_ratio, w: unused
    vec4 time;        // x: current time in seconds, yzw: unused
    
    // Ball parameters (modified)
    vec4 ball_params;  // x: unused, y: padding (radius), z: unused, w: unused
...
// Textures - just keep the noise texture
uniform sampler2D texture_sampler0;  // Noise texture for normal mapping
...
void main() {
    ...
    // Get texture aspect ratio - use a default of 1.0 if not provided
    float texture_ratio = max(resolution.z, 1.0);
    
    // Apply screen aspect ratio correction to maintain circular shape
    if (screen_ratio > 1.0) {
        // Landscape: scale x
        uv.x *= screen_ratio;
    } else {
    ...
    // Ball radius (use padding parameter)
    float radius = ball_params.y;
    
    // Fixed position at center (0,0) - remove all movement code
    vec2 ball_position = vec2(0.0, 0.0);
    ...
    // Background distortion for visual effects
    vec2 distorted_uv = uv;
    distorted_uv.x += smoothstep(0.9, 0.6, length(uv - ball_position))*amp.x*cos(frequency*t+uv.y*scale_val.x);
    distorted_uv.y += smoothstep(0.9, 0.6, length(uv - ball_position))*amp.y*sin(frequency*t+uv.x*scale_val.y);
    ...
    // Distance-based alpha for transparency
    float alpha = smoothstep(radius + 0.1, radius - 0.2, outside_purple_distance);
    ...
    vec2 normalUV = (uv - ball_position * 0.5 + 1.0) * 0.5;
    ...
    vec2 normalUV2 = (uv * 1.0 - ball_position + 1.0) * 0.5;
    ...
    color_out = vec4(color + lines, alpha);
} 